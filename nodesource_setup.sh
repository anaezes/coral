...
if [ "X${PRE_INSTALL_PKGS}" != "X" ]; then
    ...
    exec_cmd "apt-get --allow-unauthenticated install -y${PRE_INSTALL_PKGS} > /dev/null 2>&1"
fi
...
print_status """Run \`${bold}sudo apt-get --allow-unauthenticated install -y ${NODEPKG}${normal}\` to install ${NODENAME} and npm
